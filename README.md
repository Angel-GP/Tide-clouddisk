...
- 🎬 **媒体在线预览**:图片/视频/音频直接在浏览器播放(支持进度拖动、上一张/下一张、键盘切换;**视频/图片预览可用鼠标滚轮或 ↑↓ 键缩放**)
- 🖱 **悬停预览**:鼠标悬停文件名即浮出缩略图 / 静音循环视频 / 迷你播放条
- ✅ **批量管理**:列表与网格视图均有「批量管理」按钮,勾选多个文件后一键下载(自动打包 zip)/ 删除(需确认, 登录后可见)/ 隐藏(管理员可见)
...
| GET | `/preview/<path>` | 媒体在线预览(公开, 支持 Range);支持 mp4/webm/ogv/mov/avi/mkv/flv/ts/3gp/mpg/wmv、mp3/wav/flac/ogg/m4a/aac/opus/wma/aiff、jpg/png/gif/webp/bmp/ico/avif (svg 除外, 按附件下载) |
| GET | `/api/zip?p=<path>&p=<path>` | 批量打包下载(公开;隐藏项仅管理员可见) |
...
- 密码仅存哈希(随机盐),登录态通过随机 token + HttpOnly Cookie 校验,防路径穿越,隐藏文件直链拦截
- 登录失败 5 次按客户端 IP 锁定 5 分钟;反向代理场景下需在设置中勾选「信任反向代理」才读取 `X-Forwarded-For`(默认不信任,防伪造绕过)
- SVG 禁止内联渲染(一律按附件下载),所有内联预览响应带 CSP 与 nosniff
- 多用户场景按文件属主隔离写权限(覆盖/删除仅属主或管理员)
- **审计日志**(默认开启):登录成功/失败、上传、删除、重命名、隐藏、账号增删改、设置变更全部写入 `logs\audit_日期.log`,可在设置中关闭
